...
export async function runBuild(rootDir, isDev, isReleaseBuild) {
...
async function createRootPackage(opts) {
    if (opts.isDev) {
    ...
    await writeFile(indexMjsPath, `export const version = ${JSON.stringify(opts.packageJson.version)};\n`);
    await writeFile(indexDtsPath, `export declare const version: string;\n`);
}
function createBuildOptions(rootDir, isDev, isReleaseBuild) {
