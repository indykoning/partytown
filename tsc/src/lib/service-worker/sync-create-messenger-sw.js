...
const createMessengerServiceWorker = (receiveMessage) => {
    ...
    return swContainer.getRegistration().then((swRegistration) => {
        ...
        return (worker, msg) => {
            if (msg[0] === 0 /* WorkerMessageType.MainDataRequestFromWorker */) {
                ...
                worker.postMessage([1 /* WorkerMessageType.MainDataResponseToWorker */, readMainPlatform()]);
            }
                ...
                worker.postMessage([
                    3 /* WorkerMessageType.MainInterfacesResponseToWorker */,
                    readMainInterfaces(),
                ]);
            }
            else {
