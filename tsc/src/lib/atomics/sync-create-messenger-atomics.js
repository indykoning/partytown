...
const createMessengerAtomics = async (receiveMessage) => {
    ...
    return (worker, msg) => {
        ...
        if (msgType === 0 /* WorkerMessageType.MainDataRequestFromWorker */) {
            ...
            worker.postMessage([1 /* WorkerMessageType.MainDataResponseToWorker */, initData]);
        }
        else if (msg[0] === 2 /* WorkerMessageType.MainInterfacesRequestFromWorker */) {
            // web worker has requested the rest of the html/svg interfaces
            worker.postMessage([3 /* WorkerMessageType.MainInterfacesResponseToWorker */, readMainInterfaces()]);
        }
        else if (msgType === 11 /* WorkerMessageType.ForwardWorkerAccessRequest */) {
