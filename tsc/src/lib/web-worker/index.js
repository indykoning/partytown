...
const receiveMessageFromSandboxToWorker = (ev) => {
    ...
    if (webWorkerCtx.$isInitialized$) {
        if (msgType === 7 /* WorkerMessageType.InitializeNextScript */) {
            // message from main to web worker that it should initialize the next script
            initNextScriptsInWebWorker(msgValue);
        }
        else if (msgType === 9 /* WorkerMessageType.RefHandlerCallback */) {
            // main has called a worker ref handler
            callWorkerRefHandler(msgValue);
        }
        else if (msgType === 10 /* WorkerMessageType.ForwardMainTrigger */) {
            workerForwardedTriggerHandle(msgValue);
        }
        else if (msgType === 5 /* WorkerMessageType.InitializeEnvironment */) {
            createEnvironment(msgValue);
        }
        else if (msgType === 8 /* WorkerMessageType.InitializedScripts */) {
        ...
        else if (msgType === 14 /* WorkerMessageType.DocumentVisibilityState */) {
            environments[msgValue].$visibilityState$ = msg[2];
        }
        else if (msgType === 13 /* WorkerMessageType.LocationUpdate */) {
            const $winId$ = msgValue.$winId$;
            const env = environments[$winId$];
            env.$location$.href = msgValue.url;
            forwardLocationChange(msgValue.$winId$, env, msgValue);
        }
        else if (msgType === 15 /* WorkerMessageType.CustomElementCallback */) {
            callCustomElementCallback(...msg);
        }
    }
    else if (msgType === 1 /* WorkerMessageType.MainDataResponseToWorker */) {
        ...
        webWorkerCtx.$postMessage$([2 /* WorkerMessageType.MainInterfacesRequestFromWorker */]);
    }
    else if (msgType === 3 /* WorkerMessageType.MainInterfacesResponseToWorker */) {
        ...
        webWorkerCtx.$postMessage$([4 /* WorkerMessageType.InitializedWebWorker */]);
...
postMessage([0 /* WorkerMessageType.MainDataRequestFromWorker */]);
