...
export const readNextScript = (worker, winCtx) => {
    ...
    if (doc && doc.body) {
        ...
        if (!scriptElm) {
            ...
            scriptElm = doc.querySelector(scriptSelector);
        }
        if (scriptElm) {
...
export const initializedWorkerScript = (worker, winCtx, instanceId, errorMsg, scriptElm) => {
