...
const test = suite();
test('patch multiple functions on same object window.obj, queue calls', ({ win, document, navigator, }) => {
    win.partytown = {
    ...
    win.objA.objB.fn3('b');
    win.objA.objC.fn4('c');
    assert.equal(JSON.stringify(win._ptf), `[["objA","fn0"],{"0":"a"},["objA","objB","fn3"],{"0":"b"},["objA","objC","fn4"],{"0":"c"}]`);
});
test('run window.arr.push() in the web worker', ({ winId, win, env, worker, document, navigator, top, }) => {
    win.partytown = {
        forward: ['arr.push'],
    };
...
test('run window.arr.push() call after initialized', ({ winId, win, worker, document, navigator, top, }) => {
    win.partytown = {
        forward: ['arr.push'],
    };
    snippet(win, document, navigator, top, false);
...
test('patch window.obj.arr.push(), queue calls', ({ win, document, navigator }) => {
    ...
    win.a.b.c('z');
    assert.equal(JSON.stringify(win._ptf), `[["obj","arr","push"],{"0":"a","1":"b"},["a","b","c"],{"0":"z"}]`);
});
